...
{foreach $users as $user}
    <tr>
      ...
      <!--<td>{foreach $actions[$user->id] as $key=>$action}<a href="{$action.uri}"{if $action.is_ajax} class="ajaxaction" data-method="{$action.method}" data-nexthop="self"{/if}><i class="icon-key"></i></a>{/foreach}</td>-->
    </tr>
{/foreach}
  </tbody>
</table>