...
{foreach $products as $product}
    <tr>
      ...
      <td>{foreach $actions[$product->id] as $key=>$action}<a href="{$action.uri}"{if $action.is_ajax} class="ajaxaction"{/if}><img class="action_{$key}" src="{$action.img_path}"/></a>{/foreach}</td>
    </tr>
{/foreach}
  </tbody>
</table>